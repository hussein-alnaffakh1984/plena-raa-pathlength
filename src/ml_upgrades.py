...
# ---- training set ----
Ntr = 15000; print(f"simulating {Ntr} training datasets ...")
...
for ep in range(160):
    ...
    if ep % 40 == 0 or ep == 159: print(f"  epoch {ep:3d}  -logq/dim = {tot/Ntr:.3f}")
...
M, L = 500, 300; th_sbc = sample_prior(M); X_sbc = simulate(th_sbc)
...
json.dump(RES, open(os.path.join(OUT, "ml_npe_sbc.json"), "w"), indent=1, default=float)
...
fig.tight_layout(); fig.savefig(os.path.join(OUT, "ML_E_NPE_posterior.png"), dpi=140, bbox_inches="tight"); plt.close(fig)
print("saved ML_D_SBC_ranks.png, ML_E_NPE_posterior.png, ml_npe_sbc.json")
print("DONE")
